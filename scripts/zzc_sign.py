...
def zzc_sign(payload: str) -> str:
    """Generate ZZC signature for the given payload."""
    hash_hex = sha1(payload.encode('utf-8')).hexdigest().upper()

    part1 = ''.join(map(lambda i: hash_hex[i], PART_1_INDEXES))
    part2 = ''.join(map(lambda i: hash_hex[i], PART_2_INDEXES))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("Usage: ./zzc_sign.py <payload>")

...
def search_songs(
    ...
    返回：歌曲名、专辑名、收藏量；QQ 平台可点击歌名跳转热度页。
    """
    ...
    try:
        ...
        if has_raw_json:
            sel = "SELECT song_mid, name, album_name, favorite_count_text, mixsongid, raw_json FROM songs"
            mixsongid_idx, raw_json_idx = 4, 5
        else:
            sel = "SELECT song_mid, name, album_name, favorite_count_text, mixsongid FROM songs"
            mixsongid_idx, raw_json_idx = 4, None
        cur = conn.execute(
            sel + """
        ...
        "rows": [
            {
                ...
                "favorite_count": int(r[3]) if r[3] is not None else 0,
...
def get_artist_snapshot_metrics_all_platforms(
    ...
    """
    各平台「配置歌手」最新快照：粉丝数、全库歌曲收藏合计（首页饼图用）。
    ...
    for plat in SUPPORTED_PLATFORMS:
        ...
        try:
            ...
            cur = conn.execute(
                """
                SELECT COALESCE(SUM(COALESCE(favorite_count_text, 0)), 0)

...
from qqmusic_crawler.client import QQMusicClient
...
def _build_client() -> QQMusicClient:
...
def _build_snapshot_service(database_url: str) -> Tuple[QQMusicClient, CrawlerService]:
...
def _run_crawl_track_auto_pages(base_client: QQMusicClient, base_service: CrawlerService) -> None:
    ...
    # 自动按总歌曲数计算页数，未知时给一个较大上限并在空页时自动停止。
    ...
    snapshot_file = snapshots_dir / "qqmusic_{}_{}.db".format(
    ...
    result = track_changes_for_artist(
        ...
        changes_db_file=Path("data/qqmusic_changes.db"),
...
def _run_report_menu() -> None:
    ...
    for row in metric_rows:
        ...
        # Ignore initialization noise: baseline 0 -> N is not a real trend change.
        if old_value == 0:
            continue
    ...
    comment_parts = [x[2] for x in comment_items[:song_display_limit]]
    favorite_parts = [x[2] for x in favorite_items[:song_display_limit]]
    ...
    artist_parts = [x[2] for x in artist_items]
    if comment_parts:
        print("[song-names-comment]")
        for item in comment_parts:
            print("- {}".format(item))
    else:
...
def _run_check_artist_toplist(base_service: CrawlerService) -> None:
    ...
    if not resolved:
        ...
        return
    artist_mid, resolved_name = resolved
    top_n_str = input("每个榜单抓前N首(默认100): ").strip()
    try:
        top_n = int(top_n_str) if top_n_str else 100
    except ValueError:
        top_n = 100
    if top_n <= 0:
        top_n = 100
    ...
    db_file = Path("data/qqmusic_toplist.db")
    ...
    print(
        "完成: 歌手={}({}), 本次命中={}, 入库/更新={}, 库内总记录={}".format(
            resolved_name, artist_mid, len(hits), upserted, len(rows)
        )
    )
    for row in rows:
        print(
...
def _run_top_songs_menu(base_service: CrawlerService) -> None:
    ...
    snapshots_dir = Path("data/snapshots")
    candidates = sorted(snapshots_dir.glob("qqmusic_{}_*.db".format(artist_mid)))
    if not candidates:

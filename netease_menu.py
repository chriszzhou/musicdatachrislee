...
from qqmusic_crawler.netease_client import NeteaseMusicClient
...
def _build_client() -> NeteaseMusicClient:
...
def _build_snapshot_service(database_url: str) -> Tuple[NeteaseMusicClient, CrawlerService]:
...
def _run_crawl_track_auto_pages(base_client: NeteaseMusicClient, base_service: CrawlerService) -> None:
    ...
    snapshots_dir = Path("data/netease_snapshots")
    snapshots_dir.mkdir(parents=True, exist_ok=True)
    snapshot_file = snapshots_dir / "netease_{}_{}.db".format(
    ...
    result = track_changes_for_artist(
        ...
        changes_db_file=Path("data/netease_changes.db"),
...
def _run_check_artist_toplist(base_service: CrawlerService) -> None:
    ...
    db_file = Path("data/netease_toplist.db")
...
def _run_top_songs_menu(base_service: CrawlerService) -> None:
    ...
    snapshots_dir = Path("data/netease_snapshots")
    candidates = sorted(snapshots_dir.glob("netease_{}_*.db".format(artist_mid)))
    if not candidates:
